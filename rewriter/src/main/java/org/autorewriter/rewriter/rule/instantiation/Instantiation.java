...
public class Instantiation {
    ...
    private static RexNode instantiateRexNode(RexNode template, RelNode context,
        ...
        if (template instanceof RexSubQuery) {
            ...
            // Rebuild the RexSubQuery with the instantiated relational plan
            ...
            // Use clone(RelNode) to rebuild with new sub-plan, then clone(type, operands) if needed
            RexSubQuery withNewRel = sub.clone(newRel);
            if (!newOperands.equals(sub.getOperands())) {
                return withNewRel.clone(withNewRel.getType(), newOperands);
            }
            return withNewRel;
        }
